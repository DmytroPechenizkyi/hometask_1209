...
class Group:
    ...
    def serialize_to_json(self, filename: str = 'info.json'):
        with open(filename, 'w', encoding='utf-8') as file:
            students_list = []
            teachers_list = []
            serialized_dict = {'students': students_list, 'teachers': teachers_list}
            for person in self.__li:
                student_info = {}
                teacher_info = {}
                if person.get_person_type() == 'student':
                    student_info.update({'firstname': person.get_firstname(),
                                         'lastname': person.get_lastname(),
                                         'phone number': person.get_phone(),
                                         'group': person.get_group()})
                    students_list.append(student_info)
                elif person.get_person_type() == 'teacher':
                    teacher_info.update({'firstname': person.get_firstname(),
                                         'lastname': person.get_lastname(),
                                         'phone number': person.get_phone(),
